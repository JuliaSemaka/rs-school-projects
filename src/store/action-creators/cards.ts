import {
  ...
  UPDATE_CARD,
  UPDATE_CATEGORY,
} from '../reducers/cardReducer.module';
  ...
  return async (dispatch: Dispatch<IAction>) => {
    try {
      const response: Response = await fetch(
        `${Links.cards}/${indexCategory}/${page}`
      );
      dispatch({
        type: GET_CARDS_PAGE,
        payload: { indexCategory, data: await response.json() },
      });
    } catch (e) {
      throw Error(e);
    }
...
export function setLengthWords(): (
  dispatch: Dispatch<IAction>
) => Promise<void> {
  return async (dispatch: Dispatch<IAction>) => {
    try {
...
export function setLengthCategory(): (
  dispatch: Dispatch<IAction>
) => Promise<void> {
  return async (dispatch: Dispatch<IAction>) => {
    try {
