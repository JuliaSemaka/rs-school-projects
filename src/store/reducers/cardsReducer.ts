...
export const cardsReducer = (
  ...
  switch (action.type) {
    case CLEAR_CARDS:
      const newListCards = state.listCards.map((item, i) =>
    ...
    case GET_CARDS:
      const newLengthCards: number[] = action.payload.map(
        (element: ICards[]) => element.length
      );
      return {
        ...state,
        listCards: action.payload,
        isLoadingData: !!!state.categoryCards.length,
        lengthCards: newLengthCards,
      };
    case GET_CARDS_PAGE:
      const listCards = state.listCards.map((item, i) =>
        i === action.payload.indexCategory
          ? [
              ...state.listCards[action.payload.indexCategory],
              ...action.payload.data,
            ]
          : item
      );
    ...
    case GET_CATEGORIES:
      return {
        ...
        isLoadingData: !!!state.listCards.length,
        lengthCategory: action.payload.length,
      };
    case GET_CATEGORIES_PAGE:
      return {
        ...state,
        categoryCards: [...state.categoryCards, ...action.payload],
        isLoadingData: !!!state.listCards.length,
      };
    case DELETE_CARD:
    ...
    case SET_LENGTH_CARDS:
      return {
        ...state,
        lengthCards: action.payload.map((element: ICards[]) => element.length),
      };
