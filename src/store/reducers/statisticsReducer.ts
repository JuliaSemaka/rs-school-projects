...
import { ADD_ALL_STATISTIC, ADD_CLICK, ADD_CORRECT, ADD_WRONG, IStatisticsFields, IStatisticsState, KeysStatistics, RESET_STATISTICS, SORT_STATISTICS, typeSort } from "./statisticsReducer.module";
...
export const statisticsReducer = (state: IStatisticsState = baseStatistics, action: IAction): IStatisticsState => {
  switch(action.type) {
    ...
    case ADD_CORRECT:
      const newStateCorrect = state.fields.map(item => {
        if (item.word === action.payload) {
          ...
          item.procent = Math.round(100 / (parseInt(item.wrong) + parseInt(item.correct)) * parseInt(item.correct)).toString();
        }
        return item;
      });
    ...
    case SORT_STATISTICS:
      ...
      if (state.sort[key] === typeSort.DESC) {
        if (key === EKeysStatistics.category || key === EKeysStatistics.translation || key === EKeysStatistics.word) {
          sortArray = state.fields.sort((a, b) => a[key] < b[key] ? - 1 : a[key] > b[key] ? 1 : 0);
        } else {
          sortArray = state.fields.sort((a, b) => Number(a[key]) - Number(b[key]));
        }
        ...
        if (key === EKeysStatistics.category || key === EKeysStatistics.translation || key === EKeysStatistics.word) {
          ...
          sortArray = state.fields.sort((a, b) => Number(b[key]) - Number(a[key]));
        }
