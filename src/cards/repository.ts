import {ICards} from './cards.module';
...
export function getCards(): Promise<ICards[][]> {
    return Promise.resolve(listcards);
}

export function getCardsById(ind: number): Promise<ICards[] | undefined> {
    const cards = listcards.find((item,index) => index === ind);
