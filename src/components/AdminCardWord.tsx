...
const AdminCardWord: React.FC<IAdminWordPage> = ({
  item,
  index,
}: IAdminWordPage) => {
  ...
  return (
    <>
      <div
        className={`card-category card-category-word ${
          getChangeWord && 'disabled'
        }`}
      >
        <img
          className="cross"
          ...
          alt="cross"
          onClick={() => deleteCard(indexCategory, index)}
        />
        <p className="text text-title">
          Word: <span>{item.word}</span>
        </p>
        <p className="text text-title">
          Translation: <span>{item.translation}</span>
        </p>
        <p
          className="text text-title"
          onClick={() => listenAudio(item.audioSrc)}
        >
          Sound file:
          <span className="cursor-pointer">{item.audioSrc}</span>
          <img
            className="cursor-pointer"
            src={`${Links.static}images/note.png`}
            alt="note"
          />
        </p>
        <p className="text text-title">Image: </p>
        <img
          className="card-category-word-img"
        ...
        />
        <button
          className="button button-card card-category-word__change"
      ...
      {getChangeWord && (
        <AdminChangeWord
          item={item}
          index={index}
          setChangeWord={() => setChangeWord(false)}
        />
      )}
    </>
  );
