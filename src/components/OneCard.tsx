...
const OneCard: React.FC<IOneCardProps> = ({
  item,
  listenAudio,
  finishGame,
}: IOneCardProps) => {
  ...
  return (
    <div
      className={`main-card__container ${flipped && 'flipped'}`}
      onMouseLeave={() => setFlepped(false)}
    >
      <div
        className={
          !arrGameWords.length ||
          arrGameWords.find((elem) => elem.word === item.word)
            ? classesCard.join(' ')
            : classesChoiceCard.join(' ')
        }
      >
        <div className={classesMode.join(' ')}>
          <div
            className="main-card__img-fully"
            style={{
              backgroundImage: `url("${
                item.image.startsWith('data:') ? '' : Links.static
              }${item.image ? item.image : 'img/unknown-img.jpg'}")`,
            }}
            onClick={() => chouseCard(item)}
          ></div>
          <div
            className="text text-title text-center"
            onClick={() => chouseCard(item)}
          >
            {item.word}
          </div>
          <img
            className="main-card__img-rotate"
            src={`${Links.static}img/rotate2.png`}
            alt="rotate"
            onClick={() => setFlepped(true)}
          />
        </div>
