...
<div class="winners-table">
  <div class="winners-table__str winners-table__header">
    ...
    <div class="text winners-table__item sort-wins">Wins</div>
    <div class="text winners-table__item sort-best-time">Best time (second)</div>
  </div>
  <fragment>
      ${store.winners.map((item, key) => `
        ${renderWinnerLine(key + 1, item)}
      `).join('')}
    </fragment>
</div>
