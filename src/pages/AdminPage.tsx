...
export const AdminPage: React.FC = () => {
  const { categoryCards, lengthCategory } = useTypedSelector(
    (state) => state.cards
  );
      ...
      <main className="main">
        <div className="main-container">
          <InfiniteScroll
            ...
            loader={
              <div
                ...
                role="status"
                style={{ position: 'absolute', bottom: '-45px', left: '3%' }}
              >
          ...
          >
            ...
            {newCategory && (
              <AdminChangeCategory name="" changeCategory={changeNewCategory} />
            )}
            <div
              className="card-category card-category-new"
              onClick={() => setNewCategory(true)}
            >
              <p className="text text-title">Create new Category</p>
              <img
                className="plus"
                src={`${Links.static}images/plus.png`}
                alt="plus"
              />
            </div>
          </InfiniteScroll>
